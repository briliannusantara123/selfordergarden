...
.modal-content {
    ...
    width: 80%;
    max-width: 700px;
    transition: transform 1s ease, opacity 1s ease; /* Transition untuk animasi */
}

/* Gambar modal saat dihover */
.modal-content:hover {
    opacity: 0.8;
}
...
/* Animasi untuk mengecil dan bergerak keluar ke bawah */
@keyframes shrinkAndMoveOffScreen {
    0% {
        transform: scale(1) translateY(-20px); /* Posisikan gambar sedikit lebih tinggi */
        opacity: 1;
    }
    100% {
        transform: scale(0) translateY(100vh); /* Mengecil dan bergerak keluar halaman ke bawah */
        opacity: 0;
    }
}
...
<div id="myModal" class="modal">
        ...
        <img class="modal-content" id="imgModal" alt="Image not found">
    </div>
...
<script>
     ...
     // Fungsi untuk mengecek apakah gambar ada
        function checkImage(url, callback) {
            ...
            img.onload = function() {
                callback(true); // Gambar ada
            }

            img.onerror = function() {
                callback(false); // Gambar tidak ada
            }
        }

        // URL gambar yang akan dicek
        var imgUrl = '<?= base_url();?>assets/noimage.jpg'; // Ganti dengan URL gambar Anda

        // Cek apakah gambar ada
        checkImage(imgUrl, function(exists) {
            if (exists) {
                // Jika gambar ada, tampilkan modal
                var modal = document.getElementById("myModal");
                var modalImg = document.getElementById("imgModal");

                modal.style.display = "block"; // Tampilkan modal
                modalImg.src = imgUrl; // Masukkan URL gambar ke dalam modal

                // Setelah 2 detik, animasi penutupan dimulai
                setTimeout(function() {
                    // Tambahkan animasi pada gambar
                    modalImg.style.animation = "shrinkAndMoveOffScreen 1s forwards";
                    
                    // Setelah animasi selesai (1 detik), tutup modal
                    setTimeout(function() {
                        modal.style.display = "none";
                    }, 1000); // 1000ms = 1 detik sesuai durasi animasi
                }, 2000); // 2000ms = 2 detik sebelum animasi mulai
