    ...
    <div class="head">
        <header>
            <div style="display: flex; align-items: center;">
                ...
                <h2 style="margin: 0; margin-left: 5px;"><strong>Order Beverage</strong></h2>
            </div>
            <div class="profile">
                ...
                <a href="<?= base_url() ?>index.php/user/home/Minuman/<?= $nomeja ?>/<?= $s ?>" style="color: black;"><i class="fas fa-user" style="font-size: 20px;"><label style="font-size: 12px;">&nbsp;<?= $this->session->userdata('username') ?> ( <?= $this->session->userdata('nomeja') ?> )</label></i></a>
            </div>
        </header>
        <div class="search">
        ...
        <div class="categories">
          <?php foreach ($sub as $i): ?>
            <div class="category-container">
              <a href="<?= base_url() ?>index.php/orderminuman/menu/Minuman/<?= str_replace(" ","%20", $i['sub_category']) ?>#<?= str_replace(" ","_", $i['sub_category']) ?>" id="<?= str_replace(" ","_", $i['sub_category']) ?>" class="nonactive <?= str_replace(" ","_", $i['sub_category']) ?> test<?= str_replace(" ","_", $i['sub_category']) ?>"><?= $i['sub_category'] ?></a>
            </div>
          <?php endforeach ?>
        </div>
    </div>
    ...
    <?php if ($i->is_sold_out == 0): ?>
      <div class="row align-items-center menu-item position-relative">
        ...
        <div class="col-3 text-end position-relative">
          <div class="add-btn"><a href="<?= base_url() ?>index.php/orderminuman/detailmenu/<?= $i->id ?>/<?= str_replace(" ","%20", $i->sub_category) ?>#<?= str_replace(" ","_", $i->sub_category) ?>" style="padding: 3px;color: white;font-size: 25px;"><i class="bi bi-plus"></i></a></div>
        </div>
      </div>
    <?php else: ?>
      <div class="row align-items-center menu-item position-relative">
...
<div style="margin-bottom: 50px"></div>
    ...
    <footer>
        <nav>
            ...
            <a href="<?= base_url() ?>index.php/Cart/home/<?= $nomeja ?>/Minuman/<?= $s ?>" class="cart-link" style="position: relative;">
